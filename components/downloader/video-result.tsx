    ...
    <div className={cn("slide-up w-full", className)}>
      ...
      >
        <div className="flex flex-col sm:flex-row">
          {/* Thumbnail */}
          <div className="relative sm:w-40 md:w-44 flex-none aspect-video sm:aspect-auto overflow-hidden bg-(--surface-2)">
            <Image
              src={info.thumbnail}
              alt={info.title}
              fill
            ...
            {info.duration !== "—" && (
              <span className="absolute bottom-1.5 right-1.5 bg-black/80 text-white text-[9px] font-mono px-1 py-0.5 rounded whitespace-nowrap">
                {info.duration}
              </span>
            )}
            <span
              className="absolute top-1.5 left-1.5 text-[9px] font-semibold px-1 py-0.5 rounded backdrop-blur-sm whitespace-nowrap"
              style={{
          ...
          {/* Info */}
          <div className="flex-1 p-3 sm:p-3.5 flex flex-col gap-2 min-w-0">
            <div>
              <h3 className="text-[11px] sm:text-xs font-bold leading-snug line-clamp-2 text-foreground no-word-break min-w-0">
                {info.title}
              </h3>
              <p className="mt-0.5 text-[10px] text-muted-foreground truncate">{info.author}</p>
            </div>
            ...
            {info.isPlaylist && (
              <div
                className="flex items-start gap-1.5 px-2.5 py-1.5 rounded-lg text-[10px] no-word-break"
                style={{ background: "rgba(190,255,62,0.05)", border: "1px solid rgba(190,255,62,0.15)" }}
              >
                <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth={1.5} className="w-3 h-3 flex-none mt-0.5" style={{ color: "#BEFF3E", opacity: 0.7 }}>
            ...
            <div className="mt-auto flex items-center gap-1.5">
              <span className="w-1 h-1 rounded-full pulse-ring shrink-0" style={{ background: "#BEFF3E" }} />
              <span className="text-[10px] font-mono whitespace-nowrap" style={{ color: "#BEFF3E", opacity: 0.7 }}>Ready to download</span>
            </div>
          </div>
        </div>
      </div>
