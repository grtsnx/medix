...
const LOADING_LABELS: Partial<Record<DownloaderState, string>> = {
  detecting: "Interrogating your URL…",
  fetching:  "Waking up the servers…",
}
...
export function UrlInput({ url, state, platform, error, onChange, onSubmit, className }: Props) {
  ...
  return (
    <div className={cn("w-full space-y-3", className)}>
      ...
      <div
        className={cn(
          "input-ring flex items-center rounded-2xl border transition-all duration-200",
          "overflow-hidden",
          error
            ? "border-red-400/60"
            : "border-border",
      ...
      >
        ...
        <div className="flex-none pl-4 pr-2">
          {isLoading ? (
            <div className="w-4 h-4 spin-loader" />
          ) : (
            <svg
              viewBox="0 0 24 24"
              fill="none"
        ...
        <input
          ...
          onKeyDown={handleKey}
          onPaste={handlePaste}
          placeholder="Paste your link — we're professionals."
          disabled={isLoading}
          className={cn(
            "flex-1 py-4 pr-2 text-sm bg-transparent outline-none min-w-0",
            "placeholder:text-muted-foreground/60 font-mono text-foreground",
        ...
        {isKnown && !isLoading && (
          <div
            className="hidden sm:flex flex-none mx-2 items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-semibold"
            style={{
          ...
          >
            <span className="w-1.5 h-1.5 rounded-full pulse-dot" style={{ background: "var(--cobalt)" }} />
            {platformLabel}
          </div>
        )}
        ...
        <button
          ...
          className={cn(
            "btn-primary m-1.5 rounded-xl px-5 py-2.5 text-sm whitespace-nowrap",
          ...
          {isLoading ? (
            <>
              ...
              <span className="hidden sm:inline text-xs">
                {LOADING_LABELS[state] ?? "Loading…"}
              </span>
              <span className="sm:hidden text-xs">Hold on…</span>
            </>
          ) : (
            <>
      ...
      {isKnown && !isLoading && (
        <div
          className="flex sm:hidden items-center gap-1.5 text-xs font-mono px-1 animate-float-up"
          style={{ color: "var(--cobalt)" }}
        >
          <span className="w-1.5 h-1.5 rounded-full pulse-dot inline-block" style={{ background: "var(--cobalt)" }} />
      ...
      {/* Platform logo hints (idle, empty) */}
      {!hasUrl && state === "idle" && (
        <div
          className="flex items-center gap-3 px-1 flex-wrap animate-fade-in delay-400"
        ...
        >
          {BRAND_LOGOS.map((logo) => (
            <span
              key={logo.name}
              title={logo.name}
              className="flex items-center justify-center w-5 h-5 shrink-0 opacity-50 hover:opacity-80 transition-opacity"
            >
              <svg viewBox="0 0 24 24" className="w-4 h-4 block" style={{ fill: logo.color }} aria-hidden>
                <path d={logo.path} />
              </svg>
            </span>
          ))}
